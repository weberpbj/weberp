// weberp-bj v3.0 | views_ops.js | Prijevoz · Skladišta · Projekcija · Izvještaji · Plan · Plan Prodaje
// FIXES v3.0:
//   - spremiPlan: DAN se auto-izračunava iz datuma (ne iz dropdown-a)
//   - spremiPlan: TJEDAN se auto-izračunava iz datuma
//   - spremiPlan: MJESEC se auto-dodaje
//   - spremiStvarno: DAN/TJEDAN/MJESEC već ispravno izračunati (potvrđeno)
//   - openPlanModal: uklonjen dropdown za DAN (nije potreban — auto se računa)
//   - openPlanModal: datum ima min="" (može se unijeti i prošli datum za backfill)
...
function openPlanModal() {
  ...
  document.getElementById('modal-content').innerHTML = `
    ...
    <div class="form-row" style="margin-bottom:10px">
      <div class="form-group">
        ...
        <div id="p-dan-prikaz" style="font-size:11px;color:#58a6ff;margin-top:4px"></div>
      </div>
    </div>
  ...
  // Prikaz dana se ažurira čim se odabere datum
  document.getElementById('p-datum').addEventListener('change', function() {
    ...
    const dan = DANI_MAP[d.getDay()];
    const tj = getISOWeek(d);
    const el = document.getElementById('p-dan-prikaz');
    if (el) el.textContent = `→ ${DANI_PUNI[d.getDay()]} (${dan}) · Tjedan ${tj}`;
  });
  openModal();
}
...
async function spremiPlan() {
  ...
  // v3.0 FIX: DAN, TJEDAN, MJESEC se auto-izračunavaju iz datuma
  const DANI_MAP = {0:'NED',1:'PON',2:'UTO',3:'SRI',4:'ČET',5:'PET',6:'SUB'};
