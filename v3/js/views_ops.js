// weberp-bj v3.1 | views_ops.js
// FIXES v3.1:
//   FIX 1 — renderPlanProizvodnje: datum matchanje robusno (normDatum helper)
//            stv.find() koristio === na string vs Date formatima → nikad matchalo
//   FIX 2 — tjedan selector: prikazuje sve planove + filter po tjednu
//            (tekući tjedan je default ali možeš vidjeti i ostale)
//   FIX 3 — projekcija ručna korekcija: inputi se prikazuju za SVE artikle
//            (ne samo za one s prijedlog > 0, što je bio uzrok praznih kartica)
//   FIX 4 — spremiPlan: DAN/TJEDAN/MJESEC auto iz datuma (iz v3.0)
//   FIX 5 — renderPlanProizvodnje: nakon spremiStvarno čeka refresh i potvrđuje
//            u tablici da je podatak stvarno upisan (verifikacija)

// ═══════════════════════════════════════════════════════════════════
// POMOĆNE FUNKCIJE
// ═══════════════════════════════════════════════════════════════════

// Normalizira datum u YYYY-MM-DD bez obzira na format koji GAS vraća
function normDatumStr(d) {
  if (!d) return '';
  const s = String(d);
  // ISO format s vremenom: "2026-05-20T00:00:00.000Z" → "2026-05-20"
  if (s.match(/^\d{4}-\d{2}-\d{2}/)) return s.slice(0, 10);
  // Serijski broj (Excel datum)
      ...
      <tbody>
        ${zapisi.map(z=>{
          ...
          return `<tr>
            ...
            <td style="font-size:11px"><div style="font-weight:600">${z.kupac}</div><div style="color:#484f58">${z.br_narudzbe||''}</div></td>
...
// ═══════════════════════════════════════════════════════════════════
// SKLADIŠTA v3.1 — forecast 90 dana sa sliderom
// ═══════════════════════════════════════════════════════════════════
...
async function loadSkladiste() {
  ...
  <div class="card">
    ...
    <div class="tbl-wrap"><table>
      <thead><tr><th>Mj.</th><th style="text-align:right;color:#3fb950">Ulazi ↑</th><th style="text-align:right;color:#f85149">Izlazi ↓</th><th style="text-align:right">Neto</th></tr></thead>
      <tbody>
        ${obrtajMj.map(([mj,v]) => {
...
// ═══════════════════════════════════════════════════════════════════
// PROJEKCIJA v3.1 — FIX: ručna korekcija prikazuje inpute za SVE artikle
// ═══════════════════════════════════════════════════════════════════
...
function korUpdate(datum, artikl, vrijednost) {
  ...
  // Ažuriraj ukupni prikaz za taj dan
  const ukEl = document.getElementById(`proj-uk-${datum}`);
  const kapEl = document.getElementById(`proj-kap-${datum}`);
  if (ukEl && kapEl) {
    const kap = parseInt(kapEl.dataset.kap) || 45;
    const pct = Math.round(dan.prijedlog_uk / kap * 100);
    ukEl.textContent = `${dan.prijedlog_uk}/${kap} pal (${pct}%)`;
  }
}
...
function buildProjekcija(d) {
  ...
  const danHtml = (d.dani || []).map(dd => {
    ...
    // FIX 3: u korekcija modu prikazujemo SVE artikle, ne samo one s vrijednošću > 0
    const artPrikazi = _proj_korekcija ? ARTIKLI : ARTIKLI.filter(a => (dd.prijedlog_proiz || {})[a] > 0);

    const artHtml = artPrikazi.length ? artPrikazi.map(a => {
      const v = (dd.prijedlog_proiz || {})[a] || 0;
      if (_proj_korekcija) {
        return `<div class="proj-item">
          <span style="min-width:50px;font-size:10px">${a}</span>
          <input type="number" min="0" value="${v}"
            id="kor-${dd.datum}-${a.replace(/[\/ ]/g,'_')}"
            style="width:60px;background:#0d1117;border:1px solid #58a6ff44;border-radius:4px;
                   color:#ffa657;text-align:right;padding:3px 5px;font-size:11px;font-family:inherit"
    ...
    }).join('') : `<div style="font-size:9px;color:#484f58;font-style:italic">Nema prijedloga — unesi plan ili klikni Ručna korekcija</div>`;
    ...
    return `<div class="proj-day">
      ...
      <div class="proj-sec">
        ...
        ${artHtml}
        ${planUk > 0 || _proj_korekcija ? `
        <div class="pbar" style="margin-top:4px">
          <div class="pbar-f" style="width:${Math.min(100,pct)}%;background:#ffa657"></div>
        </div>
        <div id="proj-uk-${dd.datum}" style="font-size:9px;color:#7d8590;margin-top:2px"
             data-kap="${kap}">${planUk}/${kap} pal (${pct}%)</div>` : ''}
      </div>
      <div class="proj-sec">
  ...
  <div class="card" style="border-color:#58a6ff44;background:#1f3a5f22;margin-bottom:12px">
    <div style="font-size:11px;color:#58a6ff">✏️ Ručna korekcija aktivna — mijenjaj vrijednosti u karticama iznad. Klikni "Spremi u plan" kada završiš.</div>
  </div>` : ''}
  <div class="card">
...
// ═══════════════════════════════════════════════════════════════════
// PLAN PROIZVODNJE v3.1
// FIX 1: normDatumStr za robusno matchanje datuma iz GAS-a
// FIX 2: tjedan filter + mogućnost pregleda svih tjedana
// ═══════════════════════════════════════════════════════════════════
let _plan_tjedan_offset = 0; // 0 = tekući, -1 = prošli, +1 = idući

async function renderPlanProizvodnje() {
  const tjedanBaza = getISOWeek(new Date());
  const tjedan = tjedanBaza + _plan_tjedan_offset;
  ...
  // FIX 1: koristimo normDatumStr za matchanje
  const stvMatchana = stv.map(s => ({ ...s, _datum: normDatumStr(s.DATUM) }));
  const stvarUkTjedan = stvMatchana.reduce((s,r) => s + (parseInt(r.STVAR_UKUPNO)||0), 0);
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:8px">
    ...
    <div style="display:flex;gap:6px;align-items:center">
      <button class="btn btn-secondary btn-sm" onclick="_plan_tjedan_offset--;renderPlanProizvodnje()">‹ Preth.</button>
      <span style="font-size:11px;color:#7d8590">Tj. ${tjedan}</span>
      <button class="btn btn-secondary btn-sm" onclick="_plan_tjedan_offset++;renderPlanProizvodnje()">Idući ›</button>
      ${_plan_tjedan_offset!==0?`<button class="btn btn-secondary btn-sm" onclick="_plan_tjedan_offset=0;renderPlanProizvodnje()">Danas</button>`:''}
  ...
  <div class="g3" style="margin-bottom:12px">
    ...
    <div class="kpi">
      ...
      <div class="kpi-val" style="color:#3fb950" id="kpi-stvarno-uk">${stvarUkTjedan || '—'} ${stvarUkTjedan?'pal':''}</div>
    </div>
    <div class="kpi">
      <div class="kpi-label">Razlika (Δ)</div>
      <div class="kpi-val" style="color:${diffTjedan>0?'#e3b341':diffTjedan<0?'#f85149':'#3fb950'}" id="kpi-razlika">
  ...
  <div style="font-size:10px;color:#484f58;margin-bottom:8px">
    ...
    &nbsp;·&nbsp; <span style="color:#58a6ff">narančasto = plan &nbsp; zeleno = stvarno</span>
  </div>
  <div class="tbl-wrap"><table>
    <thead><tr>
    ...
    <tbody id="plan-tbody">
      ${plan.map(p => {
        // FIX 1: robusno matchanje datuma
        const pDatum = normDatumStr(p.DATUM);
        const s = stvMatchana.find(r => r._datum === pDatum) || {};
        ...
        return `<tr id="plan-row-${pDatum}">
          <td style="font-weight:700">${p.DAN}</td>
          <td style="color:#7d8590">${fmt_datum(pDatum)}</td>
          ${art.map(a => {
            ...
            return `<td style="text-align:right;font-size:11px">
              ...
              <div style="color:${stvV>0?'#3fb950':'#484f58'}">${stvV>0?stvV:'—'}</div>
            </td>`;
          }).join('')}
          <td style="color:#58a6ff;font-weight:700;text-align:right">${planUk}</td>
          <td style="color:#3fb950;font-weight:700;text-align:right" id="stv-uk-${pDatum}">${stvarUk||'—'}</td>
          <td style="color:${dlt>0?'#e3b341':dlt<0?'#f85149':'#3fb950'};font-weight:700;text-align:right" id="stv-dlt-${pDatum}">
            ${stvarUk ? ((dlt>0?'+':'')+dlt) : '—'}</td>
          <td>${zk ? '<span class="tag tag-green">Da</span>' : '<span class="tag tag-yellow">Ne</span>'}</td>
          ${mozePisati ? `<td>
            ${!zk ? `<button class="btn btn-secondary btn-sm" onclick="openStvarnoModal('${pDatum}')">Unesi stvarno</button>` : '—'}
          </td>` : ''}
...
function openPlanModal() {
  ...
  document.getElementById('modal-content').innerHTML = `
    ...
    <div class="form-row" style="margin-bottom:10px">
      <div class="form-group">
        ...
        <div id="p-dan-prikaz" style="font-size:11px;color:#58a6ff;margin-top:4px;min-height:16px"></div>
      </div>
    </div>
  ...
  document.getElementById('p-datum').addEventListener('change', function() {
    ...
    const el = document.getElementById('p-dan-prikaz');
    if (el) el.textContent = `→ ${DANI_PUNI[d.getDay()]} (${DANI_MAP[d.getDay()]}) · Tjedan ${getISOWeek(d)}`;
  });
  openModal();
}
...
function openStvarnoModal(datum) {
  ...
  document.getElementById('modal-content').innerHTML = `
    ...
    <div style="font-size:11px;color:#7d8590;margin-bottom:10px">Unesite stvarno proizvedene količine po artiklima (pal).</div>
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px">
      ${art.map(a=>`<div class="form-group"><label>${a.KOD}</label>
...
async function spremiStvarno(datum) {
  ...
  // Spremi uspjelo — zatvori modal i osvježi cijelu tablicu
  closeModal();
  toast('Stvarno upisano! Osvježavam tablicu...','success');

  // Osvježi stranicu da se povuče novo stanje iz baze
  await renderPlanProizvodnje();
}
